...
import { crearEgreso, editarEgreso } from "@/lib/actions/egresos";
import {
  METODO_PAGO_LABELS,
  TIPO_LABELS,
  type EgresoFormData,
...
export default function EgresoFormModal({
  ...
  // El estado inicial se calcula una sola vez, al montar. Antes se hacía con
  // un useEffect que llamaba setFormData en cuanto abría el modal, lo que
  // dispara un render en cascada (regla react-hooks/set-state-in-effect).
  // Ahora EgresosClient le pasa una `key` distinta en cada apertura, así que
  // el componente se remonta limpio y este cálculo vuelve a correr.

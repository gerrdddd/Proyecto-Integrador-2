"use server";
// ============================================================================
// AJUSTADO AL SCHEMA UNIFICADO:
//   - Los ENUM de MySQL ahora son MAYÚSCULAS ('EFECTIVO', 'TRANSFERENCIA',
//     'COMPLETADA'...) porque así los genera Prisma. El código original
//     comparaba contra minúsculas y no habría hecho match con ninguna fila.
//   - La tabla `egresos` ya no tiene columna `categoria`; el ajuste de
//     transferencias ahora se marca con `referencia = 'ajuste_transferencia'`.
// ============================================================================
 ...
 *
 *   ALTER TABLE ventas
 *     (ya resuelto en prisma/schema.prisma: enum MetodoPago)
 *     (ya resuelto en prisma/schema.prisma: enum EstadoVenta)
 *
 ...
 * los egresos con `referencia = 'ajuste_transferencia'` registrados en ese
...
async function obtenerCuadreTransferencia(
  ...
  // registrado con referencia = 'ajuste_transferencia' dentro del periodo.
  const [ventasFilas, ajustesFilas] = await Promise.all([
    ...
    db.$queryRaw<{ monto: number }[]>`
      ...
      FROM egresos
      WHERE fecha BETWEEN ${inicio} AND ${fin}
        AND referencia = 'ajuste_transferencia'
    `,
  ]);
